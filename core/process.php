...
$host_aceptadows = array('localhost','127.0.0.1');
...
if( in_array($_SERVER["HTTP_HOST"],$host_aceptados) ){
    ...
    if($_SERVER["REQUEST_METHOD"] == $metodo_aceptado){
        ...
        if(isset($txt_usuario) && !empty($txt_usuario)){
            //Si se enviaron los valoresen el campo usuario
            if(isset($txt_password) && !empty($txt_password)){
                //Si envio el valor de la contrasena
                if($txt_usuario==$usuario_correcto){
                    ...
                    $codigo_estado = 200;
                    $texto_estado = "OK";
                    list($usec,$sec) = esxplode('',microtime());
                    ...
                    //El valor del campo usuario no es correcto
                    $ruta = "";
        ...
        //No se acepta el metodo usado por el usuario
            $ruta = "";
            $msg = "EL CAMPO USUARIO ESTA VACIO";
            $codigo_estado = 405;
